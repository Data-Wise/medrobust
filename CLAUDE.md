...
- **Quarto caches**: `.quarto/` is gitignored (local build cache, untracked 2026-09-23). `vignettes/articles/_freeze/` is tracked, but **the site build never uses it**: pkgdown renders the articles with `vignettes/` as the Quarto project root, so `vignettes/articles/_quarto.yml` (`freeze: auto`) and its `_freeze/` are ignored and every article re-executes on each deploy (verified 2026-09-26: `gesthtn-bounds` live CIs differed from its frozen ones with the source unchanged; the frozen introduction held pre-#47 zero-width CIs the live page never showed). Only a local `quarto render` from `vignettes/articles/` reads or writes `_freeze/`. So article code must be seeded (bootstrap CIs otherwise change every build) and fast enough to run in CI; refresh `_freeze/` when you render locally so it does not go stale.
