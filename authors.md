# Authors and Citation

## Authors

- **[![ORCID iD
  icon](https://orcid.org/sites/default/files/images/orcid_16x16.png)
  Davood Tofighi](https://orcid.org/0000-0001-8523-7776)**. Author,
  maintainer, copyright holder.
  [](https://orcid.org/0000-0001-8523-7776)

## Citation

Source:
[`inst/CITATION`](https://github.com/data-wise/medrobust/blob/v0.4.4/inst/CITATION)

Tofighi D (2025). *medrobust: Robust Causal Mediation Analysis Under
Differential Misclassification*. R package version 0.4.4,
<https://github.com/data-wise/medrobust>.

    @Manual{,
      title = {medrobust: Robust Causal Mediation Analysis Under Differential Misclassification},
      author = {Davood Tofighi},
      year = {2025},
      note = {R package version 0.4.4},
      url = {https://github.com/data-wise/medrobust},
    }

Tofighi D (2025). “Partial Identification of Causal Mediation Effects
Under Differential Misclassification.” *Biostatistics*, **XX**, XXX–XXX.

    @Article{,
      title = {Partial Identification of Causal Mediation Effects Under Differential Misclassification},
      author = {Davood Tofighi},
      journal = {Biostatistics},
      year = {2025},
      volume = {XX},
      pages = {XXX--XXX},
    }
