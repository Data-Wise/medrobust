...
* Local: macOS (aarch64), R 4.6.1 — `R CMD check --as-cran --run-donttest` on the tarball built from the `v0.4.3` tag (fdc3e50): 0 errors | 0 warnings | 1 note (new submission)
* win-builder R-devel (2026-09-21 r90579): https://win-builder.r-project.org/K93SfiAxbhRT — **Status: 1 NOTE (new submission + DESCRIPTION spellings)**
* win-builder R-release (R 4.6.1): https://win-builder.r-project.org/mJgJsNKviDv0 — **Status: 1 NOTE (new submission + DESCRIPTION spellings)**
* win-builder R-oldrelease (R 4.5.3): https://win-builder.r-project.org/STAUUI0loMTW — **Status: 1 NOTE (new submission + DESCRIPTION spellings)**
* GitHub Actions: macOS-latest, ubuntu-latest, windows-latest — R release, green on the v0.4.3 tag (fdc3e50)
* r-hub (run 36188626307, on the `v0.4.3` tag):
  - `linux` (R-devel): OK
  - `windows` (R-devel): OK
  - `macos` (R-devel): OK
  - `ubuntu-clang`: OK
  ...
  - `nosuggests`: OK
  - `gcc-asan`: OK
  - `macos-arm64` (R-devel): not run — r-hub dependency setup failed again on the `v0.4.3` run (no R 4.7
